...
const CONFIG = {
  ...
  topGainers: 30,
  topLosers: 30,
  min24hVolume: 3000000,  // 3M USDT минимум
  stopLossPercent: 0.3,
...
function calculateMACD(prices) {
  ...
  // Вычисляем сигнальную линию (EMA9 от MACD)
  const macdValues = [];
...
// 8. ADX (Average Directional Index) - сила тренда
...
// ==================== КОМАНДЫ БОТА ====================
bot.start((ctx) => {
  const welcomeMessage = `🤖 <b>Binance Scalper Bot v2.0</b>
...
async function getTopMovers() {
  try {
    console.log('📡 Запрос данных с Binance...');
    ...
    console.log(`✅ Получено: ${topGainers.length} растущих, ${topLosers.length} падающих`);
...
async function analyzeSignal(pair) {
  try {
    ...
    // Подсчет подтверждений и качества сигнала
    const confirmations = [];
    let qualityScore = 0;
    ...
    if (!signal || confidence < CONFIG.minConfidence) {
      console.log(`❌ ${pair.symbol}: слабый сигнал (confidence: ${confidence.toFixed(0)}%)`);
      return null;
    }
    
    // Расчет цен входа, стопа и тейка
    ...
    if (tier === 'STANDARD') {
      console.log(`ℹ️  ${pair.symbol}: стандартный сигнал (${confidence.toFixed(0)}%)`);
      return null;
    }
    
    console.log(`✅ ${pair.symbol}: ${signal} ${tier} (${confidence.toFixed(0)}%, R:R 1:${rrRatio.toFixed(1)})`);
...
async function generateSignals() {
  try {
    ...
    return signals.slice(0, 5); // Топ-5 лучших сигналов
  } catch (error) {
...
async function start() {
  try {
    ...
    console.log('\n' + '█'.repeat(60));
    console.log('🤖 BINANCE SCALPER BOT v2.0 - ЗАПУЩЕН');
    console.log('█'.repeat(60));
    console.log('✅ Telegram бот активен');
    console.log('📡 Источник данных: Binance Public API');
    console.log('');
    ...
    // Планируем задачу каждые 20 минут
    cron.schedule('*/20 * * * *', runSignalsTask);
    
    // Первый запуск через 10 секунд
    console.log('⏳ Первое сканирование через 10 секунд...\n');
